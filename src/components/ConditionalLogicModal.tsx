...
import { ConditionalLogic, ConditionalRule, ConfiguratorOption } from '../types/ConfiguratorTypes';
import { ConditionalLogicEngine } from '../utils/ConditionalLogicEngine';

interface ConditionalLogicModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (conditionalLogic: ConditionalLogic) => void;
  currentOption: ConfiguratorOption;
...
const ConditionalLogicModal: React.FC<ConditionalLogicModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      const available = ConditionalLogicEngine.getAvailableOptionsForConditions(currentOption.id, allOptions);
  ...
  const validateLogic = () => {
    ...
    if (validation.isValid && ConditionalLogicEngine.hasCircularDependency(currentOption.id, logic, allOptions)) {
      setErrors(['Circular dependency detected. This would create an infinite loop.']);
      return false;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
      <motion.div
        ...
        className="bg-gray-800 rounded-xl border border-gray-600 shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col"
      >
        {/* Header */}
        <div className="p-6 border-b border-gray-700 bg-gray-750 rounded-t-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-white font-semibold text-xl">Conditional Logic</h3>
                <p className="text-gray-400 text-sm">Configure when "{currentOption.name}" should be visible</p>
              </div>
            </div>
            <button
              onClick={onClose}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    Control when this option appears based on other option selections
                  </p>
                </div>
                <button
            ...
            {logic.enabled && (
              <>
                ...
                {availableOptions.length === 0 ? (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-yellow-200/80 text-sm mt-1">
                          You need at least one other option to create conditional logic rules.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <>
                    {/* Check if available options have values */}
                    {availableOptions.every(opt => opt.values.length === 0) && (
                      <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-6">
                        <div className="flex items-center space-x-3">
                          <AlertTriangle className="w-6 h-6 text-orange-400" />
                          <div>
                            <h4 className="text-orange-300 font-semibold">No Option Values Available</h4>
                            <p className="text-orange-200/80 text-sm mt-1">
                              The available options don't have any values yet. Add values to other options first before creating conditional logic rules.
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={addRule}
                          disabled={availableOptions.every(opt => opt.values.length === 0)}
                      ...
                      {logic.rules.length === 0 ? (
                        <div className="text-center py-8 text-gray-500">
                          ...
                          <p className="text-sm">
                            {availableOptions.every(opt => opt.values.length === 0)
                              ? 'Add values to other options first'
                              ...
                              >
                                ...
                                <div className="mt-4 p-3 bg-gray-800 rounded-lg border border-gray-600">
                                  <p className="text-gray-300 text-sm">
                                    <span className="font-medium">Show "{currentOption.name}" when:</span>
                                    <br />
