...
import { Zap, Image as ImageIcon, Settings, Eye, EyeOff } from 'lucide-react';
import { 
  Engine, 
  Scene, 
  HemisphericLight, 
...
const BabylonPreview: React.FC<BabylonPreviewProps> = ({ 
  ...
  const [proximityHiddenComponents, setProximityHiddenComponents] = useState<Set<string>>(new Set());
  const [showProximityControls, setShowProximityControls] = useState(false);
  ...
  useEffect(() => {
    ...
    // Set up camera with your exact specifications
    const camera = new ArcRotateCamera(
      "camera",
      Math.PI / 2,
      Math.PI / 2.5,
      10, // Start zoomed comfortably at 10 units
      Vector3.Zero(),
      scene
    );
    camera.attachControl(canvas, true);
    
    // Set camera limits exactly as you specified
    ...
    camera.wheelPrecision = 50; // Natural zoom speed
    camera.pinchPrecision = 200; // Natural pinch zoom on mobile
    ...
    directionalLight2.position = new Vector3(-10, 10, -10);
  ...
  // Proximity hiding effect - monitors camera distance and hides components when too close
  useEffect(() => {
    if (!cameraRef.current || modelComponents.length === 0) return;

    const camera = cameraRef.current;
    const scene = sceneRef.current;
    if (!scene) return;

    // Define which components should be affected by proximity hiding
    const proximityAffectedComponents = [
      'inner', 'internal', 'detail', 'small', 'fine', 'mechanism', 'screw', 'bolt', 'wire', 'thread'
    ];

    const checkProximity = () => {
      const currentDistance = camera.radius;
      const newHiddenComponents = new Set<string>();

      if (currentDistance < proximityThreshold) {
        // Hide components that match proximity criteria
        modelComponents.forEach(component => {
          const componentNameLower = component.name.toLowerCase();
          const shouldHide = proximityAffectedComponents.some(keyword => 
            componentNameLower.includes(keyword)
          );

          if (shouldHide) {
            component.mesh.isVisible = false;
            newHiddenComponents.add(component.name);
          }
        });
      } else {
        // Restore all components to their configuration-based visibility
        modelComponents.forEach(component => {
          // Only restore if it was hidden by proximity, not by configuration
          if (proximityHiddenComponents.has(component.name)) {
            // Restore to the visibility state determined by configuration logic
            restoreConfigurationVisibility();
          }
        });
      }

      setProximityHiddenComponents(newHiddenComponents);
    };

    // Set up render loop observer for proximity checking
    const observer = scene.onBeforeRenderObservable.add(checkProximity);

    return () => {
      scene.onBeforeRenderObservable.remove(observer);
    };
  }, [modelComponents, proximityThreshold, proximityHiddenComponents]);

  // Function to restore configuration-based visibility
  const restoreConfigurationVisibility = useCallback(() => {
  ...
  }, [modelComponents, selectedValues, configuratorData.options]);
  ...
  useEffect(() => {
    ...
    SceneLoader.ImportMeshAsync("", "", configuratorData.model, scene).then((result) => {
      ...
      scene.executeWhenReady(() => {
        // Get all meshes except root
        const meshes = scene.meshes.filter(m => m.name !== "__root__" && m instanceof Mesh);
        ...
        // **SCALE MODEL BY 0.1 (mm to cm conversion)**
        meshes.forEach(mesh => {
          mesh.scaling = new Vector3(0.1, 0.1, 0.1);
        });

        console.log('📏 Model scaled by 0.1 (mm → cm conversion)');

        // Calculate proper bounding box for camera centering AFTER scaling
        let globalMin: Vector3 | null = null;
        let globalMax: Vector3 | null = null;
        
        // Calculate global bounding box from all meshes
        meshes.forEach(mesh => {
          if (mesh.getBoundingInfo) {
            // Get world space bounding info (after scaling)
            ...
            if (!globalMin || !globalMax) {
              ...
              globalMin = Vector3.Minimize(globalMin, min);
              globalMax = Vector3.Maximize(globalMax, max);
            }
          }
        });
        ...
        if (globalMin && globalMax) {
          ...
          // Set camera radius to 10 units (comfortable starting position)
          camera.radius = 10;
          
          console.log(`📐 Model bounds calculated (after scaling):`);
          console.log(`   Center: (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)})`);
          console.log(`   Size: (${size.x.toFixed(2)}, ${size.y.toFixed(2)}, ${size.z.toFixed(2)})`);
          console.log(`   Max dimension: ${maxDimension.toFixed(2)} cm`);
          console.log(`📷 Camera positioned at radius: ${camera.radius} units (treating 1 unit = 1 cm)`);
          console.log(`📷 Camera target: (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)})`);
        } else {
          console.warn("Unable to compute bounding info for the model.");
          // Fallback: just set camera to start position
        ...
        meshes.forEach((mesh) => {
          if (mesh.name && mesh instanceof Mesh) {
      ...
      <div className="relative" style={{ height: '50vh' }}>
        ...
        <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-md rounded-xl px-5 py-4 border border-gray-600">
          ...
          <div className="flex items-center space-x-4 text-xs text-gray-300">
            ...
            {proximityHiddenComponents.size > 0 && (
              <>
                <span>•</span>
                <span className="flex items-center">
                  <div className="w-2 h-2 bg-orange-400 rounded-full mr-1"></div>
                  Proximity Hidden: {proximityHiddenComponents.size}
                </span>
              </>
            )}
          </div>
        ...
        <div className="absolute top-4 right-4 space-y-2">
          ...
          {configuratorData.options.some(opt => opt.conditionalLogic?.enabled || opt.values.some(v => v.conditionalLogic?.enabled)) && (
            <div className="bg-purple-600/20 backdrop-blur-md rounded-xl px-4 py-3 border border-purple-500/30">
          ...
          {/* Proximity Controls */}
          <div className="bg-black/70 backdrop-blur-md rounded-xl border border-gray-600">
            <button
              onClick={() => setShowProximityControls(!showProximityControls)}
              className="w-full px-4 py-3 flex items-center justify-between text-white hover:bg-gray-700/50 transition-colors rounded-xl"
            >
              <div className="flex items-center space-x-2">
                <Settings className="w-4 h-4" />
                <span className="text-sm font-medium">Proximity Hiding</span>
              </div>
              <div className="flex items-center space-x-2">
                {proximityHiddenComponents.size > 0 && (
                ...
                <motion.div
                  animate={{ rotate: showProximityControls ? 180 : 0 }}
            ...
            {showProximityControls && (
              <motion.div
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                className="px-4 pb-3 border-t border-gray-600"
              >
                <div className="pt-3 space-y-3">
                  <div>
                    <label className="block text-gray-300 text-xs font-medium mb-2">
                      Hide Distance: {proximityThreshold.toFixed(1)} units
                    </label>
                    <input
                      type="range"
                      min="1"
                  ...
                  <div className="text-xs text-gray-400">
                    <p>Hides detailed components when camera gets too close</p>
                    <p className="mt-1">
                      Current distance: {cameraRef.current?.radius.toFixed(1) || '0'} units
                    </p>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="absolute bottom-4 right-4 bg-black/70 backdrop-blur-md rounded-xl px-4 py-3 border border-gray-600">
          <div className="text-xs text-gray-300 space-y-1">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span>Scroll to zoom (0.5-25 units)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 bg-orange-400 rounded-full"></div>
              <span>Auto-hide details when close</span>
            </div>
          </div>
        </div>
      </div>
