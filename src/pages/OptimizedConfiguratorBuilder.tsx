...
import OptimizedThreeJSPreview from '../components/OptimizedThreeJSPreview';
import ComponentSelector from '../components/ComponentSelector';
import OptimizedDragDropOption from '../components/OptimizedDragDropOption';
import OptimizedDragDropOptionValue from '../components/OptimizedDragDropOptionValue';
...
import { useOptimizedState, useOptimizedArrayState } from '../hooks/useOptimizedState';
import { PerformanceMonitor } from '../utils/PerformanceMonitor';
import { logger } from '../utils/Logger';
import { ErrorBoundary } from '../utils/ErrorBoundary';

const OptimizedConfiguratorBuilder = () => {
  const {
  ...
  const [configurators, configuratorsActions] = useOptimizedArrayState<ConfiguratorData>(
    [{
      id: 'default',
  ...
  const [activeConfiguratorId, setActiveConfiguratorId] = useOptimizedState('default', 100, 'activeConfiguratorId');
  const [selectedOptionId, setSelectedOptionId] = useOptimizedState<string | null>(null, 0, 'selectedOptionId');
  ...
  // Memoized active configurator to prevent unnecessary recalculations
  const activeConfigurator = useMemo(() => 
    configurators.find(c => c.id === activeConfiguratorId) || configurators[0],
    [configurators, activeConfiguratorId]
  );

  // Load data on component mount with performance monitoring
  useEffect(() => {
    if (!persistenceLoading) {
      const endTiming = performanceMonitor.startTiming('data-loading');
      
      try {
        const stored = loadFromStorage();
        if (stored) {
          configuratorsActions.replace(stored.configurators);
          setActiveConfiguratorId(stored.activeId);
          logger.info('Data loaded from storage', { 
            configuratorCount: stored.configurators.length,
  ...
  }, [persistenceLoading, loadFromStorage, configuratorsActions, setActiveConfiguratorId]);

  // Auto-save with debouncing and performance monitoring
  useEffect(() => {
    if (!persistenceLoading && configurators.length > 0) {
      const timer = setTimeout(() => {
        const endTiming = performanceMonitor.startTiming('auto-save');
        
        try {
          saveToStorage(configurators, activeConfiguratorId);
          logger.debug('Auto-save completed', { configuratorCount: configurators.length });
        } catch (error) {
  ...
  }, [configurators, activeConfiguratorId, persistenceLoading, saveToStorage, performanceMonitor]);

  // Optimized component loading handler
  const handleComponentsLoaded = useCallback((components: ModelComponent[]) => {
    const endTiming = performanceMonitor.startTiming('components-loaded');
    
    try {
      logger.info('🔄 Components loaded in builder', { count: components.length });
      setAvailableComponents(components);
      
      // Take memory snapshot for monitoring
      performanceMonitor.takeMemorySnapshot();
    } catch (error) {
      logger.error('Error handling components loaded', error);
    } finally {
      endTiming();
    }
  }, [performanceMonitor]);

  // Optimized configurator management functions
  const addNewConfigurator = useCallback((name: string, description: string, modelUrl: string) => {
    const endTiming = performanceMonitor.startTiming('add-configurator');
    ...
    try {
      ...
      configuratorsActions.add(newConfigurator);
      setActiveConfiguratorId(newConfigurator.id);
      setShowNewConfiguratorModal(false);
      
      logger.info('New configurator added', { id: newConfigurator.id, name });
    } catch (error) {
      logger.error('Failed to add new configurator', error);
    } finally {
      endTiming();
    }
  }, [configuratorsActions, setActiveConfiguratorId, performanceMonitor]);

  const addNewOption = useCallback((optionData: Omit<ConfiguratorOption, 'id' | 'values'>) => {
    const endTiming = performanceMonitor.startTiming('add-option');
    ...
    try {
      ...
      configuratorsActions.update(activeConfiguratorId, {
        options: [...activeConfigurator.options, newOption]
      });
      
      setShowNewOptionModal(false);
      logger.info('New option added', { optionId: newOption.id, name: newOption.name });
    } catch (error) {
      logger.error('Failed to add new option', error);
    } finally {
      endTiming();
    }
  }, [configuratorsActions, activeConfiguratorId, activeConfigurator.options, performanceMonitor]);

  // Optimized option update function
  const updateOption = useCallback((optionId: string, updates: Partial<ConfiguratorOption>) => {
    const endTiming = performanceMonitor.startTiming('update-option');
    
    try {
      const updatedOptions = activeConfigurator.options.map(option => 
        option.id === optionId ? { ...option, ...updates } : option
      );
      
      configuratorsActions.update(activeConfiguratorId, { options: updatedOptions });
      logger.debug('Option updated', { optionId, updates: Object.keys(updates) });
    } catch (error) {
      logger.error('Failed to update option', error);
    } finally {
      endTiming();
    }
  }, [activeConfigurator.options, configuratorsActions, activeConfiguratorId, performanceMonitor]);

  // Optimized option deletion with confirmation
  const deleteOption = useCallback((optionId: string) => {
    ...
    setConfirmDialog({
      ...
      type: 'danger',
      onConfirm: () => {
        const endTiming = performanceMonitor.startTiming('delete-option');
        
        try {
          const updatedOptions = activeConfigurator.options.filter(opt => opt.id !== optionId);
          configuratorsActions.update(activeConfiguratorId, { options: updatedOptions });
          ...
          logger.info('Option deleted', { optionId, name: option.name });
        } catch (error) {
  ...
  }, [activeConfigurator.options, configuratorsActions, activeConfiguratorId, selectedOptionId, setSelectedOptionId, performanceMonitor]);

  // Optimized option moving with performance monitoring
  const moveOption = useCallback((dragIndex: number, hoverIndex: number) => {
    const endTiming = performanceMonitor.startTiming('move-option');
    
    try {
      const options = [...activeConfigurator.options];
      
      if (dragIndex >= options.length || hoverIndex >= options.length) return;
      
      const draggedOption = options[dragIndex];
      options.splice(dragIndex, 1);
      options.splice(hoverIndex, 0, draggedOption);
      
      configuratorsActions.update(activeConfiguratorId, { options });
      logger.debug('Option moved', { from: dragIndex, to: hoverIndex });
    } catch (error) {
      logger.error('Failed to move option', error);
    } finally {
      endTiming();
    }
  }, [activeConfigurator.options, configuratorsActions, activeConfiguratorId, performanceMonitor]);

  // Optimized value management functions
  const addValueToOption = useCallback((optionId: string) => {
    const option = activeConfigurator.options.find(opt => opt.id === optionId);
    if (!option) return;

    const endTiming = performanceMonitor.startTiming('add-value');
    ...
    try {
      ...
      logger.info('Value added to option', { optionId, valueId: newValue.id });
    } catch (error) {
      logger.error('Failed to add value to option', error);
    } finally {
      endTiming();
    }
  }, [activeConfigurator.options, updateOption, performanceMonitor]);

  const updateOptionValue = useCallback((optionId: string, valueId: string, updates: any) => {
    const option = activeConfigurator.options.find(opt => opt.id === optionId);
    if (!option) return;

    const endTiming = performanceMonitor.startTiming('update-value');
    ...
    try {
      ...
      logger.debug('Option value updated', { optionId, valueId, updates: Object.keys(updates) });
    } catch (error) {
      logger.error('Failed to update option value', error);
    } finally {
      endTiming();
    }
  }, [activeConfigurator.options, updateOption, performanceMonitor]);
  ...
  const deleteOptionValue = useCallback((optionId: string, valueId: string) => {
    ...
    setConfirmDialog({
      ...
      type: 'danger',
      onConfirm: () => {
        const endTiming = performanceMonitor.startTiming('delete-value');
        
        try {
          const updatedValues = option.values.filter(value => value.id !== valueId);
          updateOption(optionId, { values: updatedValues });
          logger.info('Option value deleted', { optionId, valueId, name: value.name });
        } catch (error) {
          logger.error('Failed to delete option value', error);
        } finally {
          endTiming();
        }
      }
  ...
  const moveOptionValue = useCallback((optionId: string, dragIndex: number, hoverIndex: number) => {
    ...
    const endTiming = performanceMonitor.startTiming('move-value');
    ...
    try {
      ...
      logger.debug('Option value moved', { optionId, from: dragIndex, to: hoverIndex });
    } catch (error) {
      logger.error('Failed to move option value', error);
    } finally {
      endTiming();
    }
  }, [activeConfigurator.options, updateOption, performanceMonitor]);

  // Modal handlers with performance monitoring
  const openOptionEditModal = useCallback((option: ConfiguratorOption) => {
    const endTiming = performanceMonitor.startTiming('open-edit-modal');
    
    try {
      setEditingOption({ ...option });
      setShowOptionEditModal(true);
      logger.debug('Edit modal opened', { optionId: option.id });
    } catch (error) {
  ...
  const openConditionalLogicModal = useCallback((option: ConfiguratorOption) => {
    const endTiming = performanceMonitor.startTiming('open-conditional-modal');
    
    try {
      setEditingOption({ ...option });
      setShowConditionalLogicModal(true);
      logger.debug('Conditional logic modal opened', { optionId: option.id });
    } catch (error) {
      logger.error('Failed to open conditional logic modal', error);
    } finally {
      endTiming();
    }
  }, [performanceMonitor]);
  ...
  const saveOptionEdit = useCallback(() => {
    ...
    const endTiming = performanceMonitor.startTiming('save-option-edit');
    
    try {
      updateOption(editingOption.id, editingOption);
      setShowOptionEditModal(false);
      setEditingOption(null);
      logger.info('Option edit saved', { optionId: editingOption.id });
    } catch (error) {
      logger.error('Failed to save option edit', error);
    } finally {
      endTiming();
    }
  }, [editingOption, updateOption, performanceMonitor]);

  const saveConditionalLogic = useCallback((conditionalLogic: ConditionalLogic) => {
    if (!editingOption) return;
    
    const endTiming = performanceMonitor.startTiming('save-conditional-logic');
    
    try {
      updateOption(editingOption.id, { conditionalLogic });
      setEditingOption(null);
      logger.info('Conditional logic saved', { optionId: editingOption.id });
    } catch (error) {
      logger.error('Failed to save conditional logic', error);
    } finally {
      endTiming();
    }
  }, [editingOption, updateOption, performanceMonitor]);

  // Import/Export handlers with performance monitoring
  const handleExport = useCallback(async () => {
    const endTiming = performanceMonitor.startTiming('export-configurations');
    
    try {
      await exportConfigurations(configurators, activeConfiguratorId);
  ...
  }, [exportConfigurations, configurators, activeConfiguratorId, performanceMonitor]);

  const handleImport = useCallback(async () => {
    const endTiming = performanceMonitor.startTiming('import-configurations');
    ...
    try {
      ...
      setConfirmDialog({
        ...
        type: 'warning',
        onConfirm: () => {
          configuratorsActions.replace(imported.configurators);
          setActiveConfiguratorId(imported.activeId);
          logger.info('Configurations imported successfully', { count: imported.configurators.length });
        }
      });
    } catch (error) {
  ...
  }, [importConfigurations, configuratorsActions, setActiveConfiguratorId, performanceMonitor]);
  ...
  const handleClearStorage = useCallback(() => {
    setConfirmDialog({
      ...
      type: 'danger',
      onConfirm: () => {
        const endTiming = performanceMonitor.startTiming('clear-storage');
        ...
        try {
          ...
          configuratorsActions.replace([defaultConfigurator]);
          setActiveConfiguratorId('default');
          logger.info('Storage cleared and reset to defaults');
        } catch (error) {
  ...
  }, [clearStorage, configurators.length, configuratorsActions, setActiveConfiguratorId, performanceMonitor]);
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        configuratorsActions.update(activeConfiguratorId, { model: e.target.value });
                      }}
...
export default OptimizedConfiguratorBuilder;